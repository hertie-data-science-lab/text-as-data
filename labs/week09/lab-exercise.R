...
ntoken(corp) # reid is sort so we're uncertain
...
# compare and contrast: ca
library(ca)
corpmat <- as.matrix(corpdfm)
camod <- ca(corpmat)
names(camod)
head(camod$rowcoord)

# pull out the estimated coordinates
ca_theta <- camod$rowcoord[,1]
ca_beta <- camod$colcoord[,1]

plot(wfish$theta, ca_theta)
plot(wfish$beta, ca_beta)
