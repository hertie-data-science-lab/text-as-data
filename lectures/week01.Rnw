...
\vfill
\vspace{10ex}
\centerline{\Huge\Pisymbol{MinionPro-Extra}{121}}
\vfill
...
\begin{itemize}
  \item \textit{High dimensional}.  \Sexpr{used} word types (adult native english speakers know ~20-35,000)
  \item \textit{Sparse}. Of \Sexpr{as.integer(totaltypes)}, word types in total, the Conservatives only used \Sexpr{round(used, 1)}
  \item \textit{Skewed}. Of these \Sexpr{round(hapaxes, 1)} words appeared exactly once and the most frequent word \Sexpr{round(mostfreq, 1)} times
\end{itemize}
