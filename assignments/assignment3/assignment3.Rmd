...
```{r}
deuk <- filter(cmp20, edate > oldest, 
               countryname %in% c("Germany", "United Kingdom"))
...
```{r}
deuk_counts <- select(deuk, starts_with("per"))

deuk_counts[1:3, 1:7] # quick peek at the top left corner
```
and switch into a old-style `data.frame` representation that has
 ...
 about the tidyverse.
```{r}
deuk_mat <- data.matrix(deuk_counts)
rownames(deuk_mat) <- deuk$label
colnames(deuk_mat) <- itemcodes$name

deuk_mat[1:3,1:2] # top left corner of our dfm equivalent
```
...
```{r}
betas <- mod1$colcoord[,1]
thetas <- mod1$rowcoord[,1]
```

Now it's your turn. For each country, 
plot the party positions over elections. We don't have uncertainty estimates in this version of the model, so don't worry about those. You may need to do a bit of data frame construction before plotting here - 
you can get election date `edate`, and country names from `deuk`.
And if you want a list of 'main' UK parties, try
```{r}
main_parties <- c("Conservatives", "Labour", "LibDems", "SNP", "UKIP", "PC", "DUP")
```

```{r}
dat <- data.frame(position = thetas, 
                  country = deuk$countryname,
                  party = deuk$partyabbrev,
                  election = deuk$edate)

ggplot(filter(dat, country == "Germany"), 
       aes(election, position, colour = party)) + 
  geom_point() + 
  geom_line() +
  facet_grid(rows = vars(country))

ggplot(filter(dat, country == "United Kingdom",
...
So far, I hope, so familiar. Now plot the (shared) betas the same style
```{r}
dat <- data.frame(position = betas, 
                  country = names(betas))
dat <- arrange(dat, country, position) %>%
  group_by(country) %>%
  mutate(ord = 1:n())

ggplot(dat, aes(x = position, y = ord, label = party)) + 
  geom_point() +
  geom_text_repel() +
  facet_grid(country ~ .)
```
